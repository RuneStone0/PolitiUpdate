...
- **Link previews / discoverability meta (staged 2026-09-11, NOT deployed)** — no Open Graph/meta description/robots.txt/sitemap existed anywhere on the site, so every shared link rendered as a bare URL. Staged locally: `website/index.html` (meta description + OG/Twitter card + canonical), `src/digest/publisher.py` (`_render_archive_html` emits the same tags for archive pages), new `website/robots.txt` + `website/sitemap.xml`. Needs commit + push (Pages deploy) then a curl check of the served HTML — pushed only with Rune's go-ahead.
- **Website analytics (gap)** — there is no traffic measurement at all, so no channel can be evaluated and form conversion is unknown. Options: GoatCounter/Umami free tier (account → Rune) or self-hosted Umami on the UmbrelOS host (Docker, no third party). Prerequisite for judging the distribution pushes.
